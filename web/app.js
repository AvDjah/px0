...
` + text + "\n```", "Copied snippet for Agent (" + ref + ")");
  ...
  var VIM_OPTIONS_DEFAULTS = {
    hlsearch: true,
    ignorecase: false,
    relativenumber: false
  };
  var LS_KEYS = "px0.vim.keys";
  var LS_EX = "px0.vim.ex";
  var LS_OPTS = "px0.vim.opts";
    ...
    for (const mode of ["normal", "visual"]) {
      for (const [lhs, action] of Object.entries(map[mode] || {})) {
        if (!VIM_ACTIONS[action])
          problems.push(mode + ' "' + lhs + '" -> unknown action "' + action + '"');
      }
    }
  ...
  // web/src/vim.js
  var vimLS_ENABLE = "px0.vim.enabled";
  var vimEnabled = false;
  var vimModeCache = "normal";
  var vimCountBuf = "";
  var vimPending = null;
  var vimPendingTimer = 0;
  var vimKeymapCache = null;
  var vimExAliasesCache = null;
  var vimOptionsCache = null;
  var vimVisualAnchor = null;
  var vimVisualSwap = false;
  var vimLastFind = null;
  var vimLastSearch = null;
  var vimBarKind = null;
  var vimExHistory = [];
  var vimExHistIdx = -1;
  var vimSearchHistory = [];
  var vimSearchHistIdx = -1;
  var vimSearchTimer = 0;
  var vimRelObserver = null;
  function vimKeymap() {
    if (!vimKeymapCache) {
      vimKeymapCache = vimLoadKeymap();
      const v = vimValidateKeymap(vimKeymapCache);
      if (!v.ok)
        showToast("Vim", "Ignoring bad bindings: " + v.problems.slice(0, 2).join("; "));
    }
    return vimKeymapCache;
  }
  function vimExAliases() {
    if (!vimExAliasesCache)
      vimExAliasesCache = vimLoadExAliases();
  ...
  function vimIsEnabled() {
    return vimEnabled;
  }
  function vimReadEnabledPref() {
    try {
      return localStorage.getItem(vimLS_ENABLE) === "true";
    } catch {
      return false;
    }
  }
  function vimWriteEnabledPref() {
    try {
      localStorage.setItem(vimLS_ENABLE, vimEnabled ? "true" : "false");
    } catch {}
  }
  ...
  function vimUpdateChrome() {
    ...
    if (chip) {
      ...
      let label = "-- NORMAL --";
      if (vimModeCache === "visual")
        label = "-- VISUAL --";
      else if (vimModeCache === "visual-line")
        label = "-- VISUAL LINE --";
  ...
  function vimLineLen(d, n) {
    return vimLineText(d, n).length;
  }
  ...
  function vimWordUnderCursor(d) {
    ...
    if (a < t.length && isW(t[a])) {
      while (a > 0 && isW(t[a - 1]))
        a--;
      while (b < t.length && isW(t[b]))
        b++;
      return { word: t.slice(a, b), col: a };
    }
    if (a < t.length && !/\s/.test(t[a])) {
      while (a > 0 && !/\s/.test(t[a - 1]) && !isW(t[a - 1]))
        a--;
      while (b < t.length && !/\s/.test(t[b]) && !isW(t[b]))
        b++;
      return { word: t.slice(a, b), col: a };
    }
    return null;
  ...
  function vimEnsureVisible(d, center) {
    ...
    render();
    updateStatus();
    if (vimOptions().relativenumber)
      vimApplyRelativeNumbers();
  }
  function vimMoveTo(d, line, col, opts = {}) {
    ...
    if (opts.push)
      pushHistory(d.path, d.cur);
    if (vimModeCache === "visual" || vimModeCache === "visual-line")
      vimVisualUpdate();
    else
      placeCaret();
  }
  ...
  function vimFindWordFwd(d, line, col, big, count) {
    ...
    for (let k = 0;k < count; k++) {
      ...
      if (C < t.length && vimCharClass(t[C], big) !== 0) {
        ...
        while (C < t.length && vimCharClass(t[C], big) === cls)
          C++;
      }
      let found = false;
      let guard = 0;
      while (guard++ < 12000) {
        t = vimLineText(d, L);
        while (C < t.length && vimCharClass(t[C], big) === 0)
          C++;
        if (C < t.length) {
          if (big) {
            found = true;
        ...
        C = 0;
        let skip = 0;
        while (L <= d.total && vimLineText(d, L) === "" && skip++ < 500)
          L++;
        if (L > d.total)
          break;
        t = vimLineText(d, L);
      ...
      while (guard++ < 12000) {
        ...
        if (!advanced) {
          if (C + 1 < t.length) {
            ...
            advanced = true;
            let s = 0;
            while (L <= d.total && vimLineText(d, L) === "" && s++ < 500)
              L++;
            if (L > d.total)
              return k === 0 ? null : { line: L - 1, col: vimLineLen(d, L - 1) };
            continue;
          } else
            return k === 0 ? null : { line: L, col: C };
          continue;
        ...
        if (C >= t2.length) {
          if (L >= d.total)
            return { line: L, col: Math.max(0, t2.length - 1) };
          L++;
          C = 0;
          advanced = true;
          let s = 0;
          while (L <= d.total && vimLineText(d, L) === "" && s++ < 500)
            L++;
          if (L > d.total)
            return { line: L - 1, col: 0 };
          continue;
        }
        const cls = vimCharClass(t2[C], big);
        while (C + 1 < t2.length && vimCharClass(t2[C + 1], big) === cls)
          C++;
        break;
      }
    }
  ...
  function vimFindWordBack(d, line, col, big, count) {
    ...
    for (let k = 0;k < count; k++) {
      ...
      while (guard++ < 12000) {
        const t = vimLineText(d, L);
        while (C > 0 && vimCharClass(t[C] ?? " ", big) === 0)
          C--;
        ...
        const cls = vimCharClass(t[C], big);
        while (C > 0 && vimCharClass(t[C - 1], big) === cls)
          C--;
        break;
      }
    }
  ...
  function vimCharFind(d, ch, kind, count) {
    ...
    for (let k = 0;k < count; k++) {
      let idx = -1;
      if (kind === "f")
        idx = t.indexOf(ch, c + 1);
      else if (kind === "t") {
        idx = t.indexOf(ch, c + 1);
        if (idx > 0)
          idx--;
      } else if (kind === "F")
        idx = t.lastIndexOf(ch, c - 1);
      else if (kind === "T") {
        idx = t.lastIndexOf(ch, c - 1);
      ...
      if (idx < 0)
        return k === 0 ? null : { line: d.cur, col: c };
      c = idx;
    }
  ...
  function vimBlankJump(d, dir, count) {
    ...
    for (let k = 0;k < count; k++) {
      if (dir > 0) {
        let n = L;
        while (n <= d.total && isBlank(n))
          n++;
        while (n <= d.total && !isBlank(n))
          n++;
        L = Math.min(n, d.total);
      } else {
        let n = L;
        while (n >= 1 && isBlank(n))
          n--;
  ...
  function vimYank() {
    ...
    if (text) {
      ...
      const t = vimLineText(d, d.cur);
      if (t)
        copyToClipboard(t, "Yanked line " + d.cur);
      else
  ...
  async function vimDoSearch(pattern, dir, opts = {}) {
    ...
    try {
      ...
      vimBarMsg("Search error: " + e.message, true);
      return false;
    }
    const f = (j.results || []).find((r) => r.path === d.path);
    const hits = [];
    ...
    if (!hits.length) {
      if (!opts.live)
        vimBarMsg('No matches for "' + pattern + '"', true);
      S2.find = null;
      paint();
      return false;
    }
    ...
    if (vimOptions().hlsearch) {
      S2.find = { q: pattern, ci: !vimOptions().ignorecase, hits, byLine: new Set(hits.map((h) => h.line)), active: 0 };
      drawMinimap(hits, d.total);
    } else {
      S2.find = null;
    }
  ...
  function vimSearchStep(delta) {
    ...
    if (!d)
      return;
    if (S2.find && S2.find.hits && S2.find.hits.length && vimOptions().hlsearch) {
      const n = S2.find.hits.length;
      const next = ((S2.find.active + delta) % n + n) % n;
      S2.find.active = next;
      const h = S2.find.hits[next];
  ...
  function vimSearchWord(dir) {
    ...
    if (!w || !w.word) {
      showToast("!", "No word under cursor");
      return;
    }
    S2.lastWord = w.word;
  ...
  async function vimWordSearch(word, dir) {
    ...
    if (!hits.length) {
      showToast("!", 'No matches for "' + word + '"');
      return;
    }
    vimLastSearch = { q: word, dir };
    if (vimOptions().hlsearch) {
      S2.find = { q: word, ci: !vimOptions().ignorecase, hits, byLine: new Set(hits.map((h) => h.line)), active: 0 };
      drawMinimap(hits, d.total);
    }
  ...
  function vimIsVisual() {
    return vimModeCache === "visual" || vimModeCache === "visual-line";
  }
  function vimActLeft(d, count) {
    if (previewing()) {
      vimScrollPreviewBy(0);
      return;
    }
    const len = vimLineLen(d, d.cur);
  ...
  function vimActDown(d, count) {
    if (previewing()) {
      vimScrollPreviewBy(48 * count);
      d.cur = Math.min(d.total, d.cur + count);
      try {
        previewLine(d.cur);
      } catch {}
      updateStatus();
  ...
  function vimRunAction(action, count, extra) {
    ...
    switch (action) {
      ...
      case "up":
        ...
        break;
      case "wordFwd": {
        const r = vimFindWordFwd(d, d.cur, vimClampedCol(d, vimLineLen(d, d.cur)), false, count);
        if (r)
          vimMoveTo(d, r.line, r.col, {});
        break;
      }
      case "WORDfwd": {
        const r = vimFindWordFwd(d, d.cur, vimClampedCol(d, vimLineLen(d, d.cur)), true, count);
        if (r)
          vimMoveTo(d, r.line, r.col, {});
        break;
      }
      case "wordBack": {
        const r = vimFindWordBack(d, d.cur, vimClampedCol(d, vimLineLen(d, d.cur)), false, count);
        if (r)
          vimMoveTo(d, r.line, r.col, {});
        break;
      }
      case "WORDback": {
        const r = vimFindWordBack(d, d.cur, vimClampedCol(d, vimLineLen(d, d.cur)), true, count);
        if (r)
          vimMoveTo(d, r.line, r.col, {});
        break;
      }
      case "wordEnd": {
        const r = vimFindWordEnd(d, d.cur, vimClampedCol(d, vimLineLen(d, d.cur)), false, count);
        if (r)
          vimMoveTo(d, r.line, r.col, {});
        break;
      }
      case "WORDend": {
        const r = vimFindWordEnd(d, d.cur, vimClampedCol(d, vimLineLen(d, d.cur)), true, count);
        if (r)
          vimMoveTo(d, r.line, r.col, {});
        break;
      }
      case "lineStart":
        ...
        break;
      case "lineEnd":
        vimMoveTo(d, d.cur, Math.max(0, vimLineLen(d, d.cur) - (vimIsVisual() ? 0 : 1)), {});
        break;
      case "lineFirstNonBlank":
        vimMoveTo(d, d.cur, vimFirstNonBlank(d, d.cur), {});
        break;
      case "firstNonBlankNext": {
        const n = Math.min(d.total, d.cur + count);
        vimMoveTo(d, n, vimFirstNonBlank(d, n), {});
        break;
      }
      case "top": {
      ...
      case "references": {
        const w = vimWordUnderCursor(d);
        if (w)
          findReferences({ word: w.word, line: d.cur, col: w.col, path: d.path });
        else
          findReferences();
        break;
      }
      ...
      case "hoverInfo": {
        const w = vimWordUnderCursor(d);
        if (w)
          inspectReferences({ word: w.word, line: d.cur, col: w.col, path: d.path });
        else
          showToast("!", "No identifier under cursor");
        break;
      }
      case "findCharFwd":
      case "findCharBack":
      ...
      case "tillCharBack": {
        if (extra && extra.ch) {
          ...
          if (r) {
            vimMoveTo(d, r.line, r.col, {});
            vimLastFind = { ch: extra.ch, kind };
          } else
            showToast("!", "Char not found: " + extra.ch);
        } else {
      ...
      case "repeatFind": {
        if (!vimLastFind) {
          showToast("!", "No find yet (f/F/t/T first)");
          break;
        }
      ...
      case "bracketMatch":
        ...
        break;
      case "nextSymbol":
        vimSymbolJump(d, 1, count);
        break;
      case "prevSymbol":
        vimSymbolJump(d, -1, count);
        break;
      case "nextBlank":
        vimBlankJump(d, 1, count);
        break;
      case "prevBlank":
        vimBlankJump(d, -1, count);
        break;
      ...
      case "searchWordBack":
        ...
        break;
      case "viewTop": {
        const n = vimFirstVisibleLine();
        vimMoveTo(d, n, vimFirstNonBlank(d, n), {});
        break;
      }
      case "viewMiddle": {
        const n = Math.min(d.total, vimFirstVisibleLine() + Math.floor(vimViewportLines() / 2));
        vimMoveTo(d, n, vimFirstNonBlank(d, n), {});
        break;
      }
      case "viewBottom": {
        const n = Math.min(d.total, vimFirstVisibleLine() + vimViewportLines() - 1);
        vimMoveTo(d, n, vimFirstNonBlank(d, n), {});
        break;
      }
      case "centerCursor":
        vimEnsureVisible(d, true);
        break;
      case "cursorTop": {
      ...
      case "halfDown": {
        if (previewing()) {
          vimScrollPreviewBy(vp.clientHeight / 2);
          break;
        }
      ...
      case "pageDown": {
        if (previewing()) {
          vimScrollPreviewBy(vp.clientHeight * 0.9);
          break;
        }
      ...
      case "nextTab":
        if (S2.tabs.length > 1)
          switchTab((S2.active + count) % S2.tabs.length);
        break;
      case "prevTab":
        if (S2.tabs.length > 1)
          switchTab(((S2.active - count) % S2.tabs.length + S2.tabs.length) % S2.tabs.length);
        break;
      case "visualMode":
        if (vimModeCache === "visual")
          vimExitVisual();
      ...
      case "swapEnds":
        if (vimIsVisual()) {
          vimVisualSwap = !vimVisualSwap;
          vimVisualUpdate();
        }
        break;
      case "searchFwd":
        vimOpenBar("searchFwd");
        break;
      case "searchBack":
        vimOpenBar("searchBack");
        break;
      case "searchNext":
        vimSearchStep(count);
        break;
      ...
      case "exMode":
        ...
        break;
      default:
        showToast("!", "Unknown Vim action: " + action);
    }
  }
  ...
  function vimHandleKey(e) {
    ...
    if (target && (target.tagName === "INPUT" || target.tagName === "TEXTAREA"))
      return false;
    if (!$("#helpsheet")?.hidden || !$("#overlay")?.hidden)
      return false;
    if (target && (target.tagName === "BUTTON" || target.tagName === "A") && (e.key === "Enter" || e.key === " "))
      return false;
    if (e.key === "Escape" || e.ctrlKey && !e.metaKey && !e.altKey && e.key === "[") {
      if (vimBarKind) {
        vimCloseBar();
      ...
      if (S2.occ || S2.find && vimOptions().hlsearch) {
        S2.occ = null;
        try {
          clearFind();
        } catch {
  ...
  function vimOpenBar(kind, seed) {
    ...
    bar.hidden = false;
    if (prefix)
      prefix.textContent = kind === "ex" ? ":" : kind === "searchBack" ? "?" : "/";
    input.value = seed || "";
    if (msg) {
      msg.textContent = "";
    ...
    if (kind === "ex") {
      vimExHistIdx = vimExHistory.length;
    } else {
      vimSearchHistIdx = vimSearchHistory.length;
    }
    setTimeout(() => {
      input.focus();
  ...
  function vimCloseBar() {
    ...
    if (vimEnabled && vimModeCache === "cmdline")
      vimModeCache = vimVisualAnchor ? vimVisualAnchor.lineMode ? "visual-line" : "visual" : "normal";
    try {
      input?.blur();
    } catch {}
    vimUpdateChrome();
  ...
  function vimBarComplete() {
    const { input } = vimBarEls();
    if (!input || vimBarKind !== "ex")
      return;
    const cur = input.value;
    const m = /^(\s*:?\s*)(b(?:uffer)?|e(?:dit)?)\s+(\S*)$/.exec(cur);
    if (m) {
      const frag = (m[3] || "").toLowerCase();
      const names = S2.tabs.map((t) => t.path).filter((p) => p.toLowerCase().includes(frag)).slice(0, 8);
      vimBarMsg(names.length ? names.join("   ") : "No matches", !names.length);
      if (names.length === 1)
        input.value = m[1] + m[2] + " " + names[0];
      return;
    }
    const cm = /^(\s*:?\s*)(\w*)$/.exec(cur);
    if (cm) {
      const frag = cm[2].toLowerCase();
      const cmds = [...new Set([...Object.keys(vimExAliases()), "w", "q", "qa", "e", "ls", "b", "bn", "bp", "noh", "set", "map", "theme", "help", "diff"])].filter((c) => c.toLowerCase().startsWith(frag)).slice(0, 12);
  ...
  function vimAcceptBar() {
    ...
    if (vimBarKind === "searchFwd" || vimBarKind === "searchBack") {
      ...
      if (raw.trim()) {
        ...
        if (vimSearchHistory.length > 50)
          vimSearchHistory.shift();
      }
      vimCloseBar();
      if (raw)
        vimDoSearch(raw, dir, {});
  ...
  async function vimRunEx(raw) {
    ...
    if (!m) {
      vimBarMsg("Not an editor command: " + cmd, true);
      return;
    }
    let name = m[1];
    const args = (m[2] || "").trim();
    ...
    if (!canon && name.endsWith("!"))
      canon = aliases[name.slice(0, -1)];
    if (!canon) {
      vimBarMsg("Not an editor command: " + name, true);
      return;
    }
  ...
  async function vimExecEx(canon, args, rawName) {
    ...
    switch (canon) {
      ...
      case "edit": {
        ...
        let j;
        try {
          j = await api("/api/find", { q: args, limit: 10 });
        } catch (e) {
          vimBarMsg(e.message, true);
          return false;
        }
        if (!j.results || !j.results.length) {
          vimBarMsg('No file matching "' + args + '"', true);
          return false;
        }
        vimCloseBar();
        await openFile(j.results[0].path);
        if (j.results.length > 1)
          showToast("", "Opened " + j.results[0].path + " (" + j.results.length + " matches)");
        return true;
      }
      case "buffers": {
        if (!S2.tabs.length) {
          vimBarMsg("No buffers", true);
          return false;
        }
        vimBarMsg(S2.tabs.map((t, i) => i + 1 + ": " + t.path).join("   "));
        return false;
      }
      case "buffer": {
        if (!args) {
          vimBarMsg(":b needs a number or name (see :ls)", true);
          return false;
        }
        const n = parseInt(args, 10);
        if (n >= 1 && n <= S2.tabs.length) {
          vimCloseBar();
          switchTab(n - 1);
          return true;
        }
        const frag = args.toLowerCase();
        const idx = S2.tabs.findIndex((t) => t.path.toLowerCase().includes(frag) || t.name.toLowerCase().includes(frag));
        if (idx < 0) {
          vimBarMsg('No buffer matching "' + args + '"', true);
          return false;
        }
        vimCloseBar();
        switchTab(idx);
      ...
      case "bnext":
        ...
        vimBarMsg("Only one buffer", true);
        return false;
      case "bprev":
        if (S2.tabs.length > 1) {
          vimCloseBar();
      ...
      case "nohl":
        ...
        return true;
      case "set":
        return vimExSet(args);
      case "map":
        return vimExMap(args, "both");
      case "unmap":
        return vimExUnmap(args, "both");
      case "mapclear":
      ...
      case "theme": {
        ...
        if (hit && setTheme(hit.id)) {
          vimCloseBar();
          return true;
        }
        vimBarMsg("Unknown theme: " + args, true);
        return false;
      }
      case "help":
        vimCloseBar();
  ...
  function vimExSet(args) {
    ...
    for (let tok of tokens) {
      ...
      const get = () => key === "vim" ? vimEnabled : key === "wrap" ? S2.wrap : key === "number" ? S2.lineNumbers : !!opts[key];
      if (query) {
    ...
    if (opts.relativenumber && vimEnabled)
      vimApplyRelativeNumbers();
    else if (vimEnabled)
      render();
    vimBarMsg(Object.entries(opts).map(([k, v]) => (v ? "" : "no") + k).join("   "));
  ...
  function vimNormLhs(lhs) {
    return lhs.replace(/<(C|Ctrl)-([A-Za-z\[\]\\])>/gi, (_, __, k) => "C-" + k.toLowerCase()).replace(/^C-(.)$/, (_, k) => "C-" + k.toLowerCase());
  }
  function vimExMap(args, scope) {
    ...
    if (parts.length < 2) {
      vimBarMsg("Usage: :map <lhs> <action>  (e.g. :map J halfDown)", true);
      return false;
    }
    ...
    for (const m of modes)
      vimSaveKeymapDelta(m, lhs, action);
    vimKeymapCache = null;
  ...
  function vimExUnmap(args, scope) {
    ...
    if (!lhs) {
      vimBarMsg("Usage: :unmap <lhs>", true);
      return false;
    }
    const modes = scope === "both" ? ["normal", "visual"] : [scope];
    for (const m of modes)
      vimSaveKeymapDelta(m, lhs, null);
    vimKeymapCache = null;
  ...
  function vimApplyRelativeNumbers() {
    const d = doc_();
    if (!d || !vimEnabled || !vimOptions().relativenumber)
      return;
    for (const r of rowsEl.children) {
      const n = +r.dataset.l;
  ...
  function vimInitRelativeObserver() {
    if (vimRelObserver || !rowsEl)
      return;
    vimRelObserver = new MutationObserver(() => {
      if (vimEnabled && vimOptions().relativenumber)
        vimApplyRelativeNumbers();
    });
  ...
  function vimHelpHTML() {
    ...
    return '<div class="help-card"><div class="help-header"><h2>Vim Navigation</h2></div><dl class="help-grid">' + rows.map(([k, v]) => "<dt><kbd>" + k + "</kbd></dt><dd>" + v + "</dd>").join("") + '</dl><p class="hint" style="margin:12px 0 0">' + keys + "</p></div>";
  }
  function initVim() {
    ...
    document.addEventListener("input", (e) => {
      if (!vimEnabled || !vimBarKind || vimBarKind === "ex")
        return;
  ...
  function showHelp() {
    ...
    h.innerHTML = '<div class="help-card"><div class="help-header"><h2>Keyboard Shortcuts</h2>' + ver + '</div><dl class="help-grid">' + SHORTCUTS.map(([combos, v]) => "<dt>" + combos.map(keyCaps).filter(Boolean).join('<span class="key-or">/</span>') + "</dt>" + "<dd>" + esc(v) + "</dd>").join("") + "</dl></div>" + (vimIsEnabled() ? vimHelpHTML() : '<p class="hint" style="margin:12px 0 0">Vim navigation is off — Command Palette &gt; Toggle Vim Mode, footer Vim button, or :set vim (when on).</p>');
    h.hidden = false;
  }
  ...
  function initShortcuts() {
    ...
    addEventListener("keydown", (e) => {
      const mod = e[MOD];
      if (e.target && e.target.id === "vim-input") {
        if (vimIsEnabled() && vimHandleKey(e)) {
          e.preventDefault();
          e.stopPropagation();
        }
        return;
  ...
  var COMMANDS = [
    ...
    { name: "Vim: Reset Custom Key Bindings", run: () => {
      vimClearKeymapDeltas();
      location.reload();
    } },
